...
class Martfury_Shortcodes {
	...
	function icon_box_2( $atts, $content ) {
		...
		$info = vc_param_group_parse_atts( $atts['info'] );
	...
	function category_tabs( $atts, $content ) {
		...
		foreach ( $this->cat_tabs as $index => $tab ) {
			...
			$tags = vc_param_group_parse_atts( $tab['tags'] );
	...
	function banners_grid( $atts, $content ) {
		...
		if ( ! empty( $banners ) ) {
			...
			foreach ( $banners as $banner ) {
				...
				$image     = $this->get_Image_By_Size( array(
		...
		if ( ! empty( $info ) ) {
			foreach ( $info as $key => $value ) {
				...
				if ( isset( $value['image'] ) && $value['image'] ) {
					$item = $this->get_Image_By_Size( array(
	...
	function journey( $atts, $content ) {
		...
		$setting = vc_param_group_parse_atts( $atts['journey'] );
	...
	function process( $atts, $content ) {
		...
		$process = vc_param_group_parse_atts( $atts['process'] );
		$i       = 1;
		if ( $process ) {
	...
	function newsletter( $atts, $content ) {
		...
		$setting = vc_param_group_parse_atts( $atts['btn_setting'] );
	...
	function partner( $atts, $content ) {
		...
		if ( $images ) {
			...
			foreach ( $images as $attachment_id ) {
				if ( function_exists( 'martfury_get_image_html' ) ) {
					$image = martfury_get_image_html( $attachment_id, $atts['image_size'] );
				} else {
	...
	function images_grid( $atts, $content ) {
		...
		if ( $images ) {
			...
			foreach ( $images as $image ) {
				...
				$output[]   = sprintf( '<li>%s</li>', $this->get_vc_link( $link_atts ) );
			}
			$output[] = '</ul>';
		}
	...
	function products_of_category( $atts, $content ) {
		...
		if ( $el_infinite ) {
			...
			$output[] = '<div class="mf-loading"></div>';
	...
	function product_tabs( $atts, $content ) {
		...
		if ( $el_infinite ) {
			...
			$tab_content[] = '<div class="mf-loading"></div>';
	...
	function deals_of_the_day( $atts, $content ) {
		...
		$header_tabs[] = sprintf( '<div class="header-countdown">%s<div class="martfury-countdown" data-expire="%s"></div></div>', esc_html__( 'Ends in:', 'martfury' ), esc_attr( $expire ) );
		$header_tabs[] = '</div>';
		$link_atts     = array(
			'link'    => $atts['link'],
			'content' => '',
		...
		$query_args['meta_query'] = apply_filters( 'martfury_product_deals_meta_query', array_merge( WC()->query->get_meta_query(), array(
			array(
				'key'     => '_deal_quantity',
				'value'   => 0,
				'compare' => '>',
		...
		if ( $atts['cat'] ) {
			$query_args['tax_query'] = array_merge( WC()->query->get_tax_query(), array(
				array(
	...
	function get_top_selling( $atts ) {
		...
		$top_sellers = $report->get_order_report_data( array(
			'data'         => array(
				'_product_id' => array(
	...
	function banner_medium( $atts, $content ) {
		...
		$item_html = $this->get_vc_link( array(
			'link'    => $atts['link'],
			'content' => '&nbsp',
			'class'   => 'link-all',
		) );
		...
		if ( in_array( $atts['layout'], array( '2', '5' ) ) ) {
			$link_html = $this->get_vc_link( array(
				'link'    => $atts['link'],
				'content' => '',
				'class'   => 'link',
			) );
	...
	function get_links_group(
		...
		$links_group = vc_param_group_parse_atts( $links_group );
		$extra_link  = array();
