	...
	foreach ($ac->subsites as $subsite) {
		...
		if ($subsite->id == $subsite_id) {
			...
			if ($GLOBALS['DUPX_AC']->mu_mode == DUPX_MultisiteMode::Subdomain) {
				...
				$newval	 = DUPX_U::sanitize_text_field($_POST['url_new']);
                ...
                DUPX_U::queueReplacementWithEncodings($old_subdirectory_url, $post_url_new);
...
$post_path_new = DUPX_U::sanitize_text_field($_POST['path_new']);
array_push($GLOBALS['REPLACE_LIST'],
	array('search' => $post_path_old,			 'replace' => $post_path_new),
	array('search' => $path_old_json,				 'replace' => $path_new_json),
	array('search' => urlencode($post_path_old), 'replace' => urlencode($post_path_new)),
	array('search' => rtrim(DUPX_U::unsetSafePath($post_path_old), '\\'), 'replace' => rtrim($post_path_new, '/'))
);
...
if (stristr($post_url_old, 'http:') && stristr($post_url_new, 'https:') ) {
    $url_old_http = str_ireplace('https:', 'http:', $post_url_new);
    $url_new_http = DUPX_U::sanitize_text_field($_POST['url_new']);
    $url_old_http_json = str_replace('"',  "", json_encode($url_old_http));
    $url_new_http_json = str_replace('"',  "", json_encode($url_new_http));

} elseif(stristr($post_url_old, 'https:') && stristr($post_url_new, 'http:')) {
    $url_old_http = str_ireplace('http:', 'https:', $post_url_new);
    $url_new_http =DUPX_U::sanitize_text_field( $_POST['url_new']);
    $url_old_http_json = str_replace('"',  "", json_encode($url_old_http));
    $url_new_http_json = str_replace('"',  "", json_encode($url_new_http));
}
...
if (strlen($_POST['wp_username']) >= 4 && strlen($_POST['wp_password']) >= 6) {
	...
	if ($newuser_count == 0) {
		...
		$newuser1 = @mysqli_query($dbh,
				...
				VALUES ('{$wp_username}', MD5('{$wp_password}'), '{$wp_username}', '', '{$newuser_datetime}', '', '0', '{$wp_username}')");
		...
		@mysqli_query($dbh, "INSERT INTO `".mysqli_real_escape_string($dbh, $GLOBALS['DUPX_AC']->wp_tableprefix)."usermeta` (`user_id`, `meta_key`, `meta_value`) VALUES ('{$newuser1_insert_id}', 'nickname', '{$wp_username}')");
...
if (copy($wpconfig_ark_path, $wpconfig_path) === false) {
	...
	DUPX_Log::error("ERROR: Unable to copy '{$wpconfig_ark_path}' to '{$wpconfig_path}'. Check server permissions for more details see FAQ: https://snapcreek.com/duplicator/docs/faqs-tech/#faq-trouble-055-q");
