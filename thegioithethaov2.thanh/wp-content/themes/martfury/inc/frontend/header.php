...
function martfury_enqueue_scripts() {
	...
	wp_register_style( 'flex-css', get_template_directory_uri() . '/css/flexslider.css', array(), '1.0.0' );
	...
	wp_enqueue_script( 'flexslider-js', get_template_directory_uri() . '/js/jquery.flexslider-min.js', array(), time(), true );
...
function martfury_get_header_style() {
	...
	if ( ! empty( $bg_color ) ) {
		$inline_css .= '#site-header {background-color:' . $bg_color . '}';
	}

	$text_color = martfury_get_option( 'header_text_color' );
	if ( ! empty( $text_color ) ) {
		$inline_css .= '#site-header:not(.minimized) .extras-menu > li > a, #site-header:not(.minimized) .product-extra-search .hot-words li a,.site-header:not(.minimized) .mobile-menu-row .mf-toggle-menu,' .
		               '#site-header:not(.minimized) .menu-item-hotline .hotline-content,#site-header:not(.minimized) .extras-menu .menu-item-hotline .extra-icon, #site-header:not(.minimized) .extras-menu .menu-item-hotline .hotline-content label,' .
		               '#site-header .recently-viewed .recently-title, #site-header:not(.minimized) .products-cats-menu .cats-menu-title .text, #site-header:not(.minimized) .products-cats-menu .cats-menu-title{color:' . $text_color . '}';
	}
	...
	if ( ! empty( $button_bg_color ) ) {
		$inline_css .= '#site-header:not(.minimized) .product-extra-search .search-submit, #site-header:not(.minimized) .extras-menu > li > a .mini-item-counter{background-color:' . $button_bg_color . '}';

	}

	$button_color = martfury_get_option( 'search_button_text_color' );
	if ( ! empty( $button_color ) ) {
		$inline_css .= '#site-header:not(.minimized) .product-extra-search .search-submit, #site-header:not(.minimized) .extras-menu > li > a .mini-item-counter{color:' . $button_color . '}';
	...
	if ( ! empty( $menubar_border_color ) ) {
		$inline_css .= '#site-header .main-menu {border-color:' . $menubar_border_color . '}';
		$inline_css .= '.header-layout-3 .site-header .header-bar .widget:after {background-color:' . $menubar_border_color . '}';
	}
	...
	if ( ! empty( $menubar_text_color ) ) {
		$inline_css .= '#site-header .header-bar a, #site-header .recently-viewed .recently-title, .site-header .topbar a,#site-header:not(.minimized) .main-menu .products-cats-menu .cats-menu-title .text, #site-header:not(.minimized) .main-menu .products-cats-menu .cats-menu-title, #site-header .main-menu .primary-nav > ul > li > a, #site-header .main-menu .header-bar,' .
		               '#site-header .header-bar #lang_sel a.lang_sel_sel, #site-header .header-bar .lang_sel a.lang_sel_sel, #site-header .header-bar #lang_sel > ul > li > a:after, #site-header .header-bar .lang_sel > ul > li > a:after, #site-header .header-bar .mf-currency-widget .current:after{color:' . $menubar_text_color . '}';
	}

	$menubar_hover_color = martfury_get_option( 'menu_bar_hover_color' );
	if ( ! empty( $menubar_hover_color ) ) {
		$inline_css .= '#site-header .header-bar a:hover,.site-header .topbar a:hover,#site-header .primary-nav > ul > li:hover > a{color:' . $menubar_hover_color . '}';
